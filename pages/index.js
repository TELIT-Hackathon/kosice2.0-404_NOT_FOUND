                        ...
                        <li className="flex border-t border-gray-200 p-4 text-sm space-x-2">
                            ...
                            <div className="flex items-center">
                                <ul className="flex flex-col items-start">
                                    ...
                                    <li className="flex"><span className="font-semibold">rating:</span> {selectedMarker?.user?.rating}
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="#FFD700" viewBox="0 0 24 24"
                                             stroke-width="1.5" stroke="#FFD700" className="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                  d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"/>
                                        </svg>
                                    </li>
                                </ul>
                                <img src={selectedMarker?.user?.photo} alt="" className="w-14 h-14 rounded-full"/>
                            </div>
                        </li>
                        ...
                        <li className="flex  border-t border-gray-200 w-full p-4 text-sm space-x-2">
                            ...
                            <div className="flex-1">
                                <ul className="flex flex-col">
                                    <li>
                                        <span className="font-semibold">duration:</span> {selectedMarker?.duration}
                                    </li>
                                    <li>
                                        <span className="font-semibold">distance:</span> {selectedMarker?.distance}
                                    </li>
                                    <li>
                                        <span className="font-semibold">average CO2 cost:</span> {selectedMarker?.co2}
                                    </li>
                        ...
                        <li className="flex justify-between border-t border-gray-200 p-4 text-sm space-x-2">
                            ...
                            <div className="flex-1">
                                <ul className="flex flex-col text-end w-fit items-start">
                                    ...
                                    {selectedMarker?.schedule && Object.keys(selectedMarker?.schedule).map(day => {
                                        return <li key={day}><span className="font-semibold">{day}</span>: {selectedMarker?.schedule?.[day] ?? 'no ride'}</li>
                                    })}
